...
WPVSurvey.helpers = {
	...
	if (document.documentElement.clientWidth < 680) {
	    ...
	    if (!haveMovePrev) {
		jQuery('.device-guide-container').before('<span class="move-prev">Prev</span>');
	    }
	    if (!haveMoveNext) {
		jQuery('.device-guide-container').after('<span class="move-next">Next</span>');
	    }
	...
	jQuery('.move-next').click(function() {
	    jQuery(this).prev('.device-guide-container').find('ul').css('float', 'right');
	    jQuery(this).hide();
	    jQuery(this).prevAll('.move-prev').show();
	});
	jQuery('.move-prev').click(function() {
	    jQuery(this).next('.device-guide-container').find('ul').css('float', 'none');
	    jQuery(this).hide();
	    jQuery(this).nextAll('.move-next').show();
	});
	//add class for checked styles
	jQuery('.device-guide li label').click(function() {
	...
	var i = 2;
	jQuery('.device-guide').each(function() {
	    jQuery(this).find('ul').append('<li class="answer-btn"><div>On to Question <span>' + i++ + '</span><br />of 3</div></li>');
	});
	jQuery('.device-guide ul:last li:last').addClass("device-guide-submit").html('<div>Take a look at<br /><br /><span>Your<br />Perfect<br />Phone</span></div>');
	...
	jQuery('.device-guide-submit').click(function() {
	    var selections = [];
	    var guideID;
	    var checked;
	    ...
	    guideID = jQuery('.device-guide-container').attr('data-id');
	    ...
	    jQuery('#guide-results').hide();
	    var guide = {action: "product_guide_results", post_id: guideID, data: selections};
	    jQuery.ajax({
		type: "post",
		url: wp_ajax.ajaxurl,
		data: guide,
		...
		success: function(response) {
		    ...
		    jQuery('#device-connection p.dc-subhead').slideUp();
		    jQuery('#guide-results p.featured-desc').html(response['description']);
			...
			jQuery('.device-guide-submit').closest('.question-group').fadeOut({duration: 'slow', complete: function(){
				jQuery('#guide-results').fadeIn({duration:'fast'});
			}});
		    });
		}
	    });
